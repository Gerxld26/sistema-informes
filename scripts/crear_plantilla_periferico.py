...
RUTA_SALIDA = RAIZ / "plantillas" / "plantilla_informe_tecnico_periferico.docx"
...
p_area = parrafo(espacio_despues=6)
r1 = p_area.add_run("\u00c1REA")
r1.bold = True
r2 = p_area.add_run("\t\t:\t")
r3 = p_area.add_run("{{ area }}")
r3.bold = True

parrafo("{{ intro }}", espacio_despues=6)

parrafo("DESCRIPCION DEL PROBLEMA:", espacio_despues=4, negrita=True)

parrafo("{% for grupo in grupos_problema %}", es_tag=True)
parrafo("{{ grupo.titulo }}", espacio_despues=2, negrita=True)
parrafo("{% for punto in grupo.puntos %}", es_tag=True)
parrafo("{{ punto }}", espacio_despues=1, sangria=1, negrita=True, vineta="\u27a2")
parrafo("{% endfor %}", es_tag=True)
parrafo(espacio_despues=2)
parrafo("{% endfor %}", es_tag=True)

parrafo("CONCLUSION:", espacio_despues=4, negrita=True)
parrafo("{{ conclusion }}", espacio_despues=4)

parrafo("RECOMENDACIONES:", espacio_despues=2, negrita=True)
parrafo("{% for rec in recomendaciones %}", es_tag=True)
parrafo("{{ rec }}", espacio_despues=2, sangria=1, vineta="\u27a2")
parrafo("{% endfor %}", es_tag=True)
